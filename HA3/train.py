...
class GAN(pl.LightningModule):
    def __init__(
        ...
        if self.truncation_trick:
            ...
            self.val_noise.data =torch.from_numpy(stats.truncnorm.rvs(-1, 1,size=(len(self.train_dataset), self.noise_channels)))
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        noise=torch.randn(labels.shape[0],self.noise_channels, device=device)
        inputs_dis=self.forward(noise, labels)
        
        if optimizer_idx == 0:
            loss = self.loss(self.dis(inputs_dis, labels))
        if optimizer_idx == 1:
            loss = self.loss(self.dis(inputs_dis, labels),self.dis(imgs, labels))
