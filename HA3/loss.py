    ...
    def forward(self, fake_scores, real_scores=None):
        if real_scores is None:
            ...
            if self.loss_type=='non_saturating':
                ...
                real_label = torch.ones(fake_scores.shape,device=device)
                loss=F.binary_cross_entropy_with_logits(fake_scores, real_label)
            ...
            if self.loss_type=='non_saturating':
                device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                real_label = torch.ones(real_scores.shape,device=device)
                fake_label = torch.zeros(fake_scores.shape,device=device)
                loss=F.binary_cross_entropy_with_logits(real_scores, real_label) + F.binary_cross_entropy_with_logits(fake_scores, fake_label)

            if self.loss_type=='hinge':
                loss=-torch.mean(torch.minimum(torch.zeros_like(real_scores),real_scores-1))-torch.mean(torch.minimum(torch.zeros_like(fake_scores),-fake_scores-1))
...
class ValLoss(nn.Module):
    ...
    def calc_fid(real_features, fake_features):
        ...
        norm=torch.linalg.norm(torch.mean(real_features,dim=0)-torch.mean(fake_features,dim=0))**2
        trace=torch.trace(torch.cov(real_features)+torch.cov(fake_features)-torch.tensor(2*linalg.sqrtm(torch.matmul(torch.cov(real_features),torch.cov(fake_features))).real))
        fid=norm+trace
        return fid.item() # TODO (2 points)

    @staticmethod
    def calc_is(fake_probs):
        fake_probs=torch.tensor(fake_probs)
        is_=torch.exp(torch.mean(torch.sum((fake_probs*(torch.log(fake_probs+0.00000000001)-torch.log(torch.mean(fake_probs,dim=0, keepdim=True)+0.00000000001))),dim=1)))
        return is_.item()# TODO (2 points)
